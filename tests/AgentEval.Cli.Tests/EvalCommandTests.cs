...
public class EvalCommandTests
{
    ...
    {
        // Navigate from bin/Debug/net9.0 up to repo root, then to test datasets
        ...
        if (dir == null)
            throw new InvalidOperationException("Could not find repo root (AgentEval.Cli.slnx)");

        return Path.Combine(dir.FullName, "tests", "AgentEval.Cli.Tests", "datasets", "rag-qa.yaml");
    }
